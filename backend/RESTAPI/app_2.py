...
def get_data():
              ...
              "aggs": {
                "latest_record": {
                  "top_hits": {
                    ...
                    "_source": {
                      "includes": ["predicted_price", "prediction_time"]
                    },
                    "size": 1
                  }
                }
    ...
    try:
        ...
        index_financial = "new-finhub-data-6"
        financial_response = es.search(index=index_financial, body=daily_query_real)
        ...
        financial_response_predicted = es.search(index=index_financial, body=daily_predictions)
        formatted_financial_data_prediction = []

        buckets = financial_response_predicted['aggregations']['by_day']['buckets']
        for bucket in buckets:
            if bucket['latest_record']['hits']['total']['value'] > 0:
                latest_record = bucket['latest_record']['hits']['hits'][0]['_source']
                predicted_price = latest_record.get('predicted_price', None)
                prediction_time = latest_record.get('prediction_time', None)
                formatted_financial_data_prediction.append({
        ...
        response = {
            ...
            "news": formatted_news,
            "latest_financial_real": formatted_financial_data,
            "latest_financial_prediction":formatted_financial_data_prediction 
        }
